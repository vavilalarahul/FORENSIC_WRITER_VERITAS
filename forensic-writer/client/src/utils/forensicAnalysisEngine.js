...
class ForensicAnalysisEngine {
    ...
    /**
     * Parse and analyze evidence files
     */
    async analyzeEvidence(files, caseInfo) {
        this.analysisResults = {
            evidence: [],
            patterns: [],
            anomalies: [],
    ...
    identifyCriticalFindings() {
        ...
        // High significance correlations
        const highSignificanceCorrelations = this.analysisResults.correlations.filter(c => c.significance === 'high');
        highSignificanceCorrelations.forEach(correlation => {
            criticalFindings.push({
                type: 'correlation',
                description: correlation.description,
                severity: 'high',
                evidence: correlation.data.files?.join(', ') || 'Multiple sources',
                impact: 'Cross-system activity patterns detected requiring investigation'
            });
        });
        ...
        criticalInsights.forEach(insight => {
            criticalFindings.push({
                ...
                impact: 'Immediate action required to prevent potential security incident'
            });
        });
    ...
    extractCallDuration(line) {
        const match = line.match(/(\d+)\s*(?:seconds?|secs?|s)/);
    ...
    extractCallDirection(line) {
        const lower = line.toLowerCase();
        if (lower.includes('incoming') || lower.includes('received')) return 'incoming';
        if (lower.includes('outgoing') || lower.includes('dialed')) return 'outgoing';
        return 'unknown';
    }
    ...
    calculateAverageDuration(calls) {
        const validDurations = calls.filter(c => c.duration).map(c => c.duration);
    ...
    determineEventSeverity(entry) {
        ...
        if (entry.level === 'warning') return 'medium';
        if (entry.ip && this.isUnusualTime(entry.timestamp)) return 'medium';
        return 'low';
    }
}

export default ForensicAnalysisEngine;
