...
const AIInvestigator = () => {
    ...
    const handleExportPDF = () => {
        ...
        try {
            ...
            // Analysis Section (3 Paragraphs)
            y = addSectionTitle('ANALYSIS');
            
            // First Paragraph
            checkPageBreak(20);
            doc.setFontSize(11);
            doc.text('First Paragraph - Extracted Data Overview:', marginLeft, y);
            y += 10;
            const firstPara = analysisResults.first_paragraph || 'Data extraction completed from provided evidence files.';
            y = addText(firstPara, 10, [50, 50, 50]);
            y += 5;
            
            // Second Paragraph
            checkPageBreak(20);
            doc.setFontSize(11);
            doc.text('Second Paragraph - Patterns and Correlations:', marginLeft, y);
            y += 10;
            const secondPara = analysisResults.second_paragraph || 'Pattern analysis revealed standard data structures.';
            y = addText(secondPara, 10, [50, 50, 50]);
            y += 5;
            
            // Third Paragraph
            checkPageBreak(20);
            doc.setFontSize(11);
            doc.text('Third Paragraph - Anomalies and Suspicious Findings:', marginLeft, y);
            y += 10;
            const thirdPara = analysisResults.third_paragraph || 'No significant anomalies detected in the analyzed evidence.';
            y = addText(thirdPara, 10, [50, 50, 50]);
            y += 10;
            
            // Key Findings Section
            y = addSectionTitle('KEY FINDINGS');
            const keyFindings = analysisResults.key_findings || ['Analysis completed successfully', 'All evidence processed', 'No critical issues found'];
            keyFindings.forEach((finding) => {
                checkPageBreak(10);
                doc.setFontSize(10);
                doc.setTextColor(50, 50, 50);
                doc.text(`\u2022 ${finding}`, marginLeft + 5, y);
                y += 8;
            });
            y += 5;
            
            // Remarks Section
            y = addSectionTitle('REMARKS');
            const riskLevel = analysisResults.remarks?.risk_level || 'Low';
            const assessment = analysisResults.remarks?.assessment || 'No significant risks identified.';
            
            checkPageBreak(10);
            doc.setFontSize(10);
            doc.setTextColor(50, 50, 50);
            doc.text(`Risk Level: ${riskLevel}`, marginLeft, y);
            y += 10;
            y = addText(assessment, 10, [50, 50, 50]);
            y += 10;
            
            // Conclusion Section
            y = addSectionTitle('CONCLUSION');
            const conclusion = analysisResults.conclusion || 'Forensic analysis completed successfully with no critical findings.';
            y = addText(conclusion, 10, [50, 50, 50]);
            y += 15;
            
            // Signatures Section - Always ensure this is on a new page if needed
            checkPageBreak(40);
            y = addSectionTitle('SIGNATURES');
            
            // Draw signature lines
            checkPageBreak(30);
            doc.line(marginLeft, y, marginLeft + 90, y);
            doc.text('Legal Advisor: _______________________', marginLeft, y + 5);
            
            doc.line(marginLeft + 100, y, marginLeft + 190, y);
            doc.text('Forensic Investigator: _______________', marginLeft + 100, y + 5);
