...
export function Architecture() {
  return (
    <section className="py-32 bg-[#0B1220] relative border-t border-[#1F2937] overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom_right,rgba(34,211,238,0.1),transparent_50%)] pointer-events-none" />
        ...
        <div className="flex flex-col items-center max-w-xl mx-auto">
          {architectureNodes.map((node, idx) => (
            <div key={idx} className="w-full flex flex-col items-center">
              ...
              >
                {/* Subtle internal glow matching node color */}
                <div 
                ...
                <div 
                  className="p-4 rounded-xl bg-[#0B1220] border border-[#1F2937] shadow-inner"
                  style={{ color: node.color, boxShadow: `inset 0 0 15px ${node.color}20` }}
                >
                  {node.icon}
                </div>
                <span className="text-[#E5E7EB] font-bold text-xl font-['Space_Grotesk'] tracking-wide">
                  {node.label}
                </span>
              </motion.div>
              ...
              {idx < architectureNodes.length - 1 && (
                ...
                >
                  ...
                  <div className="absolute top-0 bottom-0 w-[2px] bg-gradient-to-b from-transparent via-[#22D3EE] to-transparent shadow-[0_0_10px_#22D3EE] animate-pulse" />
                  <ArrowDown size={32} className="text-[#22D3EE] z-10 drop-shadow-[0_0_8px_#22D3EE]" />
                </motion.div>
              )}
            </div>
          ))}
